...
# Salida: tests/mutantes-resultado.txt (una línea por mutante, con sus rojos)
set -uo pipefail
cd "$(dirname "$0")/.."
SALIDA="tests/mutantes-resultado.txt"
: > "$SALIDA"
...
echo "── auditando (cada mutante ≈ 3-4 min) ──"

mutante "M1-auth-sin-token" servicio.py \
  'GATE = [Depends(auth)]' 'GATE = []'

mutante "M2-censo-vacio" ledger_parse.py \
  'AGENTES, DUENO, DIFUSION, ESCUCHA = _censo()' \
  'AGENTES, DUENO, DIFUSION, ESCUCHA = ([], {}, [], {})'

mutante "M3-get-consume-cursor" servicio.py \
  '    # Los nombres cuyo correo cae aquí: el suyo y los que escuche por censo.' \
  '''    con.execute("INSERT OR REPLACE INTO cursors VALUES (?,?,?,?)",
                (lp.canonico(agent), list(LEDGERS)[0], 99999, "x")); con.commit()
    # Los nombres cuyo correo cae aquí: el suyo y los que escuche por censo.'''

# ⚠️ La 1ª versión de M4 y M7 usaba `[] or [...]`, que en Python DEVUELVE la lista de
# la derecha: eran no-ops, y su «ningún rojo» no decía NADA del gate. Un mutante que
# no muta da exactamente la misma señal que un gate ciego — es la forma más fácil de
# publicar un hallazgo falso sobre tu propia suite. Ahora sí borran el efecto.
